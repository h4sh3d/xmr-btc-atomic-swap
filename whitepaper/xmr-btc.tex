...
This protocol is inspired on a Monero StackExchange post discussing if it's possible to trade Monero and Bitcoin in a trustless manner \cite{MoneroStackexchangeSwap}. The concept is roughly the same, with changes in Bitcoin construction, less prerequisites in Monero, and more detailed explainations.
...
proves that a valid discrete logarithm $\alpha$, given two groups with fixed generators $G$ and $H$, where $\alpha \leq min(|G|,|H|)$, exists such that given $xG$ and $yH$: $x=y=\alpha$. Groups are \texttt{edward25519} with $|G| = l$ and \texttt{secp256k1} with $|H| = n$.
...
Key exchange is performed with \texttt{ECDSA} one-time VES---also adaptor signature---, as demonstated in \cite{oneTimeVES}, and equal discrete logarithm across groups zero-knowledge proof of knowledge\cite{MRL0010}. One-time VES are constructed such that given the ciphertext and the decrypted signature, the decryption key is easily recoverable. By setting the echanged key---one half of the Monero private spend key---as the decryption key we have a way to atomically sell the
exchanged key to the other participant. Thus, because Bitcoin and Monero use different curves, we need to prove the relation between a point on \texttt{edward25519} and \texttt{secp256k1} to ensure a trustless protocol.
...
\begin{equation}
\begin{split}
    \exists k^s_i \mid K^s_i = k^s_iG \land B^s_i = k^s_iH \land k^s_i < min(l,n)
\end{split}
\end{equation}
    ...
    \begin{table}[H]
      ...
      \begin{tabular}{ | l c l | }
        \hline
          ...
          %$\sigma_r \coloneqq (\sigma, \sigma')$ & & \\

          \multicolumn{3}{|c|}{$\xrightarrow{\langle \sigma_r'', \sigma_2' \rangle}$} \\
          %\multicolumn{3}{|c|}{$\sigma_r \coloneqq (\sigma_r', \sigma_r'')$} \\
          ...
          %\multicolumn{3}{|c|}{$m \coloneqq \BTX_\textit{buy}$} \\

          %$m \coloneqq \BTX_\textit{buy}$ & & \\
          ...
          %spend $\BTX_\textit{lock}$ w/ $\langle k^s_a, b_a, s \rangle$ & & \\
      ...
      \end{tabular}}
    \end{table}
  %\caption{Protocol definition of a successful cross-chain atomic swap between Alice and Bob, without refund procedure.}
  \label{fig:protocol}
\end{figure}
